...
import { Layers, Box, GitMerge, Flame, Cog, Star, Wrench, Cpu, Settings, ArrowRight } from "lucide-react";
    ...
    <main className="bg-white">
      ...
      <div className="bg-[#0F172A] py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <span className="text-xs font-semibold text-[#0A66C2] uppercase tracking-wider">What We Offer</span>
          <h1 className="text-4xl md:text-5xl font-bold text-white mt-2 font-['Chivo']">Our Services</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SERVICES.map((s) => (
              <div
                key={s.title}
                className="bg-white border border-slate-200 rounded-md p-7 hover:border-[#0A66C2] hover:-translate-y-1 hover:shadow-md transition-all duration-300"
                data-testid={`service-detail-${s.title.replace(/\s+/g, "-").toLowerCase()}`}
              >
                <div className="w-16 h-16 bg-blue-50 rounded-md flex items-center justify-center text-[#0A66C2] mb-5">
                  {s.icon}
                </div>
                <h3 className="text-xl font-bold text-slate-900 mb-1 font-['Chivo']">{s.title}</h3>
                <p className="text-xs font-semibold text-[#0A66C2] uppercase tracking-wide mb-3">{s.subtitle}</p>
                <p className="text-sm text-slate-600 leading-relaxed mb-5">{s.desc}</p>
                <div className="flex flex-wrap gap-2">
                  {s.sectors.map((sector) => (
                    <span key={sector} className="text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full">{sector}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="mt-16 bg-slate-50 border border-slate-200 rounded-md p-10 text-center">
            <h2 className="text-2xl font-bold text-slate-900 mb-3 font-['Chivo']">Need a Custom Solution?</h2>
            <p className="text-slate-600 mb-6">Tell us your requirements and we'll provide a tailored quote.</p>
            <Link
              to="/contact"
